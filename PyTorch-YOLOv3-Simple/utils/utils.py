...
def rescale_boxes_video(boxes, current_dim, original_shape):
    """ Rescales bounding boxes to the original shape """
    orig_h, orig_w = original_shape
    # The amount of padding that was added
    pad_x = max(orig_h - orig_w, 0) * (current_dim / max(original_shape))
    ...
    boxes[0][:, 0] = ((boxes[0][:, 0] - pad_x // 2) / unpad_w) * orig_w
    boxes[0][:, 1] = ((boxes[0][:, 1] - pad_y // 2) / unpad_h) * orig_h
    boxes[0][:, 2] = ((boxes[0][:, 2] - pad_x // 2) / unpad_w) * orig_w
    boxes[0][:, 3] = ((boxes[0][:, 3] - pad_y // 2) / unpad_h) * orig_h

    return boxes



def rescale_boxes_video1(boxes, current_dim, original_shape, pad):
    """ Rescales bounding boxes to the original shape """
    orig_h, orig_w = original_shape
    ...
    if orig_h < orig_w:
        pad_x = 0
        pad_y = pad
        '''cur_h = current_dim[0] - pad_y
        cur_w = current_dim[1]

        pad_x = pad_x * (cur_w / original_shape[1])
        pad_y = pad_y * (cur_h / original_shape[0])'''
    else:
        pad_x = pad
        pad_y = 0
        '''cur_h = current_dim[0]
        cur_w = current_dim[1] - pad_x
